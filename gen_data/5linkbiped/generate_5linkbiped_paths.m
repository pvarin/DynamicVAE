...
%clc; clear; 
%addpath ../../
savepath = 'D:\Kevin\school_stuff\5linkbiped_data\data.mat';
trials = 1000;
data_t = zeros(trials,30);
data_q = zeros(trials,5,30);
data_u = zeros(trials,5,30);
data_dq = zeros(trials,5,30);
...
% Parameters to vary:
% stepLength, stepTime, tibia_mass, femur_mass, torso_mass, tibias_length,
% femur_length, torso_length, tibia_dist, femur_dist, torso_dist

param = getRandomizedPhysicalParameters();    
...
problem.bounds.initialstate.low = [qLow; zeros(5,1)];
problem.bounds.initialstate.upp = [qLow; zeros(5,1)];
problem.bounds.finalstate.low = [qUpp; zeros(5,1)];
problem.bounds.finalstate.upp = [qUpp; zeros(5,1)];
...
if mod(trial,50)==0
    save(savepath,'data_t','data_q','data_u','data_dq','trial');
end
